...
Q16  Computed Pattern  : reconciliation du compteur num_mflix_comments
Q17  Computed Pattern  : correction par bulk_write / UpdateOne
Q18  Subset Pattern    : embarquer les 3 commentaires les plus recents

Pre-requis :  pip install "pymongo>=4.6"
Lancement  :  python patterns.py
=====================================================================
NOTE SUR LE PORT — mon conteneur mongo-ipssi (repris du Jour 1) est
publie sur le port hote 27018 et non 27017 : un service MongoDB local
occupait deja 27017 sur ma machine. Le port INTERNE reste 27017, donc
toutes les commandes `docker exec` de l'enonce sont inchangees.
La variable MFLIX_URI permet de surcharger l'URI sans toucher au code.
"""
...
# --- Connexion : UN SEUL MongoClient global, reutilise partout -------
# Le MongoClient gere son propre pool de connexions : en creer un par
# appel de fonction serait un anti-pattern (nouveau handshake + nouveau
# pool a chaque fois).
URI = os.environ.get(
    "MFLIX_URI",
